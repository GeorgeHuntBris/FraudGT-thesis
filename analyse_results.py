...
def analyse(prefix, best_metric="f1"):
    ...
    print(f"\n{'Model':<30} {'F1 (Val)':<16} {'Prec (Val)':<16} {'Rec (Val)':<16} {'AUC (Val)':<16}  |  {'F1 (Test)':<16} {'Prec (Test)':<16} {'Rec (Test)':<16} {'AUC (Test)':<16}")
    print("-" * 185)
    ...
    for model in models:
        ...
        if m["f1"]:
            print(f"{name:<30} {fmt(m['f1']):<16} {fmt(m['precision']):<16} {fmt(m['recall']):<16} "
                  f"{fmt(m['auc']):<16}  |  {fmt(m['test_f1']):<16} {fmt(m['test_precision']):<16} "
                  f"{fmt(m['test_recall']):<16} {fmt(m['test_auc']):<16}")
        else:
